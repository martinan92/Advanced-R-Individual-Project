...
```{r}
glmnet_cv<-cv.glmnet(x = data.matrix(clean_train_df[, !'price']), nfolds = 5, y = clean_train_df[['price']],
                     alpha=1, family = 'gaussian', standardize = T)
plot.cv.glmnet(glmnet_cv)

glmnet_cv$lambda.min

glmnet_0<-glmnet(x = data.matrix(clean_train_df[, !'price']), 
...
test_glmnet<-predict(glmnet_0, newx = as.matrix(clean_test_df[, !'price']))
test_glmnet

df_pred<-clean_test_df[, .(id=1:.N,price, test_glmnet)]
str(df_pred)
...
rmse_glmnet<-rmse(real=clean_test_df, predicted = test_glmnet)
mae_glmnet<-mae(real=clean_test_df, predicted = test_glmnet)
mape_glmnet<-mape(real=clean_test_df, predicted = test_glmnet)
mape_glmnet
```
...
```{r}
baseline_rf <- ranger(formula = as.formula(price~.), data=clean_train_df[,!c('id')],num.trees=400)
print(baseline_rf)

test_rf<-predict(baseline_rf, data = clean_test_df, type='response')
...
rmse_rf<-rmse(real=clean_test_df, predicted = test_rf)
mae_rf<-mae(real=clean_test_df, predicted = test_rf)
mape_rf<-mape(real=clean_test_df, predicted = test_rf)
mape_rf
```
...
xgb_reg_0<-xgboost(booster='gblinear',
                   data=as.matrix(clean_train_df[, !'price', with=F]),
                   label=raw_train_df$price,
                   nrounds = 100,
                   objective='reg:linear')
print(xgb_0)

test_xgb<-predict(xgb_0, newdata = as.matrix(clean_test_df[, !'price', with=F]), type='response')
...
rmse_xgb<-rmse(real=clean_test_df, predicted = test_xgb)
mae_xgb<-mae(real=clean_test_df, predicted = test_xgb)
mape_xgb<-mape(real=clean_test_df, predicted = test_xgb)
mape_xgb
```
