...
pacman::p_load("ggplot2","data.table","lubridate","bit64","rpart","partykit","dplyr","leaflet","chron","proj4","ranger", "ggplot2", "ggthemes", "tidyr", "dplyr", "xts", "glmnet", "xgboost", "tuneRanger", "mlr", "Hmisc", "gridExtra", "breakDown")
...
A number of features will be created in the hopes their inclusion into the model will improve the overall prediction abilities. The features created include 1) Weekday/Weekend flag, 2) Holiday flag, 3) Renovation flag (defined as when the 2015 area of either the lot or living room is different from the original area), 4) Missing Renovation Year flag (as the presence of a renovation year should correspond to a positive renovation flag), and 5) House Age. It was found that only the inclusion of the first feature actually improved model performance as seen from the below graphs. 
...
result<-data.table(method=c('baseline','fe1','fe2','fe3','fe4','fe5','final'),
...
final_rf <- ranger(formula = as.formula(price~.), data=fe_train_df1[,!c('id')], importance = 'impurity')
print(final_rf)

final_test_rf<-predict(final_rf, data = fe_test_df1, type='response')$predictions
...
explanation <- broken(final_rf, fe_train_df1[100,-2], data = fe_train_df1[,-2],
                    predict.function = prediction_function, 
                    direction = "down")

plot(explanation) + ggtitle("breakDown plot  (direction=down) for ranger model")
```
